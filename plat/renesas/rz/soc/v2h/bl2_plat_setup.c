...
int bl2_plat_handle_pre_image_load(unsigned int image_id)
{
	if (image_id == BL31_IMAGE_ID) {
		bl2_to_bl31_params_mem_t *params = (bl2_to_bl31_params_mem_t *)RZV2H_PARAMS_BASE;
		...
		if (params->boot_kind == RZ_WARM_BOOT) {
			...
			flush_dcache_range((uintptr_t)RZV2H_PARAMS_BASE, sizeof(bl2_to_bl31_params_mem_t));
...
int bl2_plat_handle_post_image_load(unsigned int image_id)
{
	...
	if (!params) {
		params = (bl2_to_bl31_params_mem_t *) RZV2H_PARAMS_BASE;
		memset((void *)RZV2H_PARAMS_BASE, 0, sizeof(bl2_to_bl31_params_mem_t));
	}
...
void bl2_el3_plat_arch_setup(void)
{
	const mmap_region_t bl2_regions[] = {
		...
		MAP_REGION_FLAT(RZV2H_PARAMS_BASE, RZV2H_PARAMS_SIZE,
				MT_MEMORY | MT_RW | MT_SECURE),
