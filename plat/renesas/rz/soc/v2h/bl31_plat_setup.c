...
{
	/* Read model from QSPI for RZ/V2H*/
	uintptr_t model_addr = (uintptr_t )(RZV2H_XSPI_MEMORY_MAP_BASE + RZV2H_BOARD_INFO_QSPI_OFFSET + OFFSET_MODEL_ID);
	uint32_t model = mmio_read_32(model_addr);

	/* Read revision id from QSPI for RZ/V2H*/
	uintptr_t revision_addr = (uintptr_t )(RZV2H_XSPI_MEMORY_MAP_BASE + RZV2H_BOARD_INFO_QSPI_OFFSET + OFFSET_REVISION);
	uint32_t revision = mmio_read_32(revision_addr);
