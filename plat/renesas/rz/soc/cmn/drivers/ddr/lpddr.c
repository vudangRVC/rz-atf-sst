...
static void save_retcsr(void)
{
	...
	if (!saved) {
		...
		prop = fdt_getprop(fdt, ddr_node, "retention_phyreglist_1d", &len);
		for (i = 0; i < (len / sizeof(uint32_t)); i++, j++) {
			ddr_csr_table[j] = dwc_ddrphy_apb_rd(fdt32_to_cpu(prop[i]));
		}
		
		prop = fdt_getprop(fdt, ddr_node, "retention_phyreglist_2d", &len);
		for (i = 0; i < (len / sizeof(uint32_t)); i++, j++) {
			ddr_csr_table[j] = dwc_ddrphy_apb_rd(fdt32_to_cpu(prop[i]));
		}
		
		prop = fdt_getprop(fdt, ddr_node, "retention_mcreglist", &len);
		for (i = 0; i < (len / sizeof(uint32_t)); i++, j++) {
			ddr_csr_table[j] = ddrtop_mc_apb_rd(fdt32_to_cpu(prop[i]));
		}
...
static void restore_retcsr(void)
{
	...
	prop = fdt_getprop(fdt, ddr_node, "retention_phyreglist_1d", &len);
	for (i = 0; i < (len / sizeof(uint32_t)); i++, j++) {
		dwc_ddrphy_apb_wr(fdt32_to_cpu(prop[i]), ddr_csr_table[j]);
	}
