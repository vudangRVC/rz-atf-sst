...
static EMMC_ERROR_CODE emmc_set_ext_csd(uint32_t arg);
static EMMC_ERROR_CODE emmc_set_request_mmc_clock(uint32_t *freq);
static EMMC_ERROR_CODE emmc_trans_sector(uint32_t *buff_address_virtual);
static uint32_t emmc_bit_field(uint8_t *data, uint32_t top, uint32_t bottom);
static uint32_t emmc_calc_tran_speed(uint32_t *freq);
static uint32_t emmc_interrupt(void);
static uint32_t emmc_set_timeout_register_value(uint32_t freq);
static void emmc_get_partition_access(void);
static void emmc_make_nontrans_cmd(HAL_MEMCARD_COMMAND cmd, uint32_t arg);
static void emmc_set_bootpartition(void);
static void emmc_write_error_info_func_no(uint16_t func_no);
static void emmc_write_error_info(uint16_t func_no, EMMC_ERROR_CODE error_code);
...
static EMMC_ERROR_CODE emmc_exec_cmd(uint32_t error_mask, uint32_t *response)
{
...
static EMMC_ERROR_CODE emmc_send_idle_cmd(uint32_t arg)
{
...
static EMMC_ERROR_CODE emmc_mount(void)
{
	EMMC_ERROR_CODE result;
...
static EMMC_ERROR_CODE emmc_memcard_power(uint8_t mode)
{
...
static EMMC_ERROR_CODE emmc_init(void)
{
	EMMC_ERROR_CODE result;
